...
2. Install `cloudflared` from [Cloudflare's official downloads](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/). If it is in `.local-tools/cloudflared.exe`, Thiri starts and reconnects the tunnel automatically. Otherwise, in a second PowerShell window run `powershell -ExecutionPolicy Bypass -File scripts/start-remote-access.ps1`.
